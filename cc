	...
	while True:
		...
		for _ in range(conn - len(socket_list)):
			...
			try:
				...
				pass
nums = 0
def checking(lines,socks_type,ms):#Proxy checker coded by PurpleHaze..x3
	global nums
	global proxies
...
def check_socks(ms):#Coded by PurpleHaze..x3
	global nums
	thread_list=[]
